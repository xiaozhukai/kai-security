package com.github.kai.security.gate.filter;
...
import com.github.kai.security.common.util.ClientUtil;
...
public class SessionAccessFilter extends ZuulFilter{
    ...
    public Object run() {
        ...
        if (user != null) {
            ...
            //设置头部校验信息
            ctx.addZuulRequestHeader("Authorization", Base64Utils.encodeToString(user.getUsername().getBytes()));   // 查找合法链接
        }
...
//        log.debug("uri：" + requestUri + "----method：" + method);
     ...
     * CreateTime: 23:19
     */
    private Collection<PermissionInfo> getPermissionInfos(String requestUri, String method, List<PermissionInfo> serviceInfo) {
    ...
    private User getSessionUser(HttpSession httpSession){
        Session session = repository.getSession(httpSession.getId());   //获取session
